(function () {
  ...
  function normalizeObject(rawObject, index) {
    ...
    return {
      ...
      locked: object.locked === true || undefined,
  ...
  function createObjectNode(rawObject, index) {
    ...
    node.setAttribute("data-object-index", String(index));
    if (object.locked) node.setAttribute("data-object-locked", "true");
  ...
  function normalizeStyleMap(value) {
    ...
    Object.keys(value).forEach(function (path) {
      var style = normalizeElementStyle(value[path]);
      if (Object.keys(style).length) styles[path] = style;
    });
    return styles;
  }
  ...
  function normalizeElementStyle(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) return {};
    var style = {};
    var fontSize = Number(value.fontSize);
    var borderWidth = Number(value.borderWidth);
    var borderRadius = Number(value.borderRadius);
    var opacity = Number(value.opacity);
    var color = normalizeStyleColor(value.color);
    var backgroundColor = normalizeStyleColor(value.backgroundColor);
    var borderColor = normalizeStyleColor(value.borderColor);
    ...
    if (isFinite(borderWidth) && borderWidth >= 0) style.borderWidth = clamp(Math.round(borderWidth), 0, 24);
    if (isFinite(borderRadius) && borderRadius >= 0) style.borderRadius = clamp(Math.round(borderRadius), 0, 96);
    if (isFinite(opacity)) style.opacity = Math.round(clamp(opacity, 0.1, 1) * 100) / 100;
  ...
  function normalizeStyleAlign(value) {
    return ["left", "center", "right", "justify"].indexOf(value) !== -1 ? value : "";
  }

  function normalizeStyleWeight(value) {
    var weight = String(value || "").trim();
  ...
  function applyElementStyles(article, slide) {
    ...
    article.querySelectorAll("[data-ppt-path]").forEach(function (node) {
      applyElementStyle(node, styles[node.getAttribute("data-ppt-path")] || {});
    });
  }
  ...
  function applyCanvasOffsets(article, slide) {
    ...
    article.querySelectorAll("[data-ppt-path]").forEach(function (node) {
      ...
      if (!x && !y && !w && !h) return;
      if (!node.classList.contains("ppt-textbox") && window.getComputedStyle(node).position === "static") {
  ...
  function normalizeSlide(raw, index) {
    ...
    slide.styles = normalizeStyleMap(slide.styles);
    return slide;
  }
  ...
  function standaloneSource() {
    return "(function(){'use strict';" +
      "var d=JSON.parse(document.getElementById('ppt-html-data').textContent),i=0,w=1280,h=720,t=0,anim=0,blank='',jump='',jt=0,ct=0,mode='fit';" +
      ...
      "function obj(o,j,s){o=o||{};var d=o.data||{},ty=o.type||'shape';if(ty==='line'||ty==='arrow'||ty==='rectangle'||ty==='roundedRectangle'||ty==='ellipse'||ty==='callout'){if(!d.kind)d.kind=ty;ty='shape'}var n=e('div','ppt-object ppt-object-'+ty);n.setAttribute('data-ppt-path','objects.'+j);n.setAttribute('data-object-type',ty);n.setAttribute('data-object-index',j);n.style.left=Math.round(num(o.x))+'px';n.style.top=Math.round(num(o.y))+'px';n.style.width=Math.max(24,Math.round(num(o.w)||320))+'px';n.style.height=Math.max(24,Math.round(num(o.h)||180))+'px';n.style.zIndex=String(num(o.zIndex)||8);if(num(o.rotation))n.style.rotate=num(o.rotation)+'deg';sty(n,o.style||{});sty(n,(s.styles&&s.styles['objects.'+j])||{});if(ty==='image')n.appendChild(media(d));else if(ty==='video')n.appendChild(vid(d));else if(ty==='audio')n.appendChild(aud(d));else if(ty==='chart')n.appendChild(chart(d));else if(ty==='table')n.appendChild(tbl(d));else if(ty==='cards')n.appendChild(ocards(d));else if(ty==='metrics')n.appendChild(ometrics(d));else if(ty==='timeline')n.appendChild(otimeline(d));else if(ty==='shape')n.appendChild(shape(d,j));else if(ty==='quote'){a(n,'p','ppt-quote',d.quote);a(n,'p','ppt-author',d.author)}else if(ty==='code')n.appendChild(e('pre','ppt-code',d.code||''));else if(ty==='compare'){[d.left||{},d.right||{}].forEach(function(o){var ca=e('section','ppt-compare-card');a(ca,'h2','',o.title);a(ca,'p','',o.text);n.appendChild(ca)})}else n.appendChild(e('div','ppt-image-placeholder',d.label||'Object'));return n}" +
      "function objs(n,s){arr(s.objects).forEach(function(o,j){if(o&&o.hidden)return;n.appendChild(obj(o,j,s||{}))})}" +
      "function tp(n,p){if(n)n.setAttribute('data-ppt-path',p)}function ino(x){while(x){if(x.classList&&x.classList.contains('ppt-object'))return true;x=x.parentNode}return false}function neo(n,s){return Array.prototype.filter.call(n.querySelectorAll(s),function(x){return !ino(x)})}" +
      "function otp(n,j,ty){var p='objects.'+j+'.data.';if(ty==='image'||ty==='video'||ty==='audio'){tp(n.querySelector('.ppt-caption'),p+'caption');return}if(ty==='chart'){var donut=n.querySelector('.ppt-chart-kind-donut');n.querySelectorAll('.ppt-chart-legend-item strong').forEach(function(x,k){tp(x,p+(donut?'labels.'+k:'series.'+k+'.name'))});if(donut)n.querySelectorAll('.ppt-chart-legend-item small').forEach(function(x,k){tp(x,p+'series.0.values.'+k)});return}if(ty==='table'){n.querySelectorAll('.ppt-table th').forEach(function(x,k){tp(x,p+'columns.'+k)});n.querySelectorAll('.ppt-table tbody tr').forEach(function(r,ri){r.querySelectorAll('td').forEach(function(x,ci){tp(x,p+'rows.'+ri+'.'+ci)})});return}if(ty==='cards'){n.querySelectorAll('.ppt-card').forEach(function(c,k){tp(c.querySelector('h2'),p+'cards.'+k+'.title');tp(c.querySelector('p'),p+'cards.'+k+'.text')});return}if(ty==='metrics'){n.querySelectorAll('.ppt-metric').forEach(function(c,k){tp(c.querySelector('strong'),p+'metrics.'+k+'.value');tp(c.querySelector('span'),p+'metrics.'+k+'.label');tp(c.querySelector('p'),p+'metrics.'+k+'.detail')});return}if(ty==='timeline'){n.querySelectorAll('.ppt-time-item').forEach(function(c,k){tp(c.querySelector('h2'),p+'items.'+k+'.title');tp(c.querySelector('p'),p+'items.'+k+'.text')});return}if(ty==='compare'){['left','right'].forEach(function(side,k){var c=n.querySelectorAll('.ppt-compare-card')[k];if(c){tp(c.querySelector('h2'),p+side+'.title');tp(c.querySelector('p'),p+side+'.text')}});return}if(ty==='quote'){tp(n.querySelector('.ppt-quote'),p+'quote');tp(n.querySelector('.ppt-author'),p+'author');return}if(ty==='code'){tp(n.querySelector('.ppt-code'),p+'code');return}if(ty==='shape')tp(n.querySelector('.ppt-shape-text'),p+'text')}" +
      ...
      "function off(n,s){tp(n.querySelector('.ppt-kicker'),'kicker');tp(n.querySelector('.ppt-title'),'title');tp(n.querySelector('.ppt-subtitle'),'subtitle');tp(n.querySelector('.ppt-body'),'body');tp(n.querySelector('.ppt-media'),'image');tp(n.querySelector('.ppt-caption'),'image.caption');tp(n.querySelector('.ppt-video'),'video');if(n.querySelector('.ppt-video .ppt-caption'))tp(n.querySelector('.ppt-video .ppt-caption'),'video.caption');tp(n.querySelector('.ppt-audio'),'audio');if(n.querySelector('.ppt-audio .ppt-caption'))tp(n.querySelector('.ppt-audio .ppt-caption'),'audio.caption');tp(neo(n,'.ppt-chart-wrap')[0],'chart');tp(neo(n,'.ppt-table')[0],'table');tp(neo(n,'.ppt-card-grid')[0],'cards');tp(neo(n,'.ppt-metric-grid')[0],'metrics');tp(neo(n,'.ppt-timeline')[0],'timeline');n.querySelectorAll('.ppt-textbox').forEach(function(x,j){tp(x,'textBoxes.'+j+'.text')});n.querySelectorAll('.ppt-object').forEach(function(x,j){var oi=Number(x.getAttribute('data-object-index'));if(!isFinite(oi))oi=j;tp(x,'objects.'+oi);otp(x,oi,x.getAttribute('data-object-type'))});if(s.layout==='quote'){tp(neo(n,'.ppt-quote')[0],'quote');tp(neo(n,'.ppt-author')[0],'author')}neo(n,'.ppt-list li').forEach(function(x,j){tp(x,'items.'+j+'.text')});var ccards=neo(n,'.ppt-compare-card');['left','right'].forEach(function(side,j){var c=ccards[j];if(c){tp(c.querySelector('h2'),side+'.title');tp(c.querySelector('p'),side+'.text')}});neo(n,'.ppt-card').forEach(function(c,j){tp(c.querySelector('h2'),'cards.'+j+'.title');tp(c.querySelector('p'),'cards.'+j+'.text')});neo(n,'.ppt-time-item').forEach(function(c,j){tp(c.querySelector('h2'),'items.'+j+'.title');tp(c.querySelector('p'),'items.'+j+'.text')});neo(n,'.ppt-metric').forEach(function(c,j){tp(c.querySelector('strong'),'metrics.'+j+'.value');tp(c.querySelector('span'),'metrics.'+j+'.label');tp(c.querySelector('p'),'metrics.'+j+'.detail')});neo(n,'.ppt-table th').forEach(function(c,j){tp(c,'table.columns.'+j)});neo(n,'.ppt-table tbody tr').forEach(function(r,ri){r.querySelectorAll('td').forEach(function(c,ci){tp(c,'table.rows.'+ri+'.'+ci)})});neo(n,'.ppt-chart-legend-item strong').forEach(function(x,j){tp(x,s.chart&&s.chart.kind==='donut'?'chart.labels.'+j:'chart.series.'+j+'.name')});if(s.chart&&s.chart.kind==='donut')neo(n,'.ppt-chart-legend-item small').forEach(function(x,j){tp(x,'chart.series.0.values.'+j)});tp(neo(n,'.ppt-code')[0],'code');appp(n,s)}" +
      ...
      "var root=document.getElementById('ppt-player-root'),stage=e('div','ppt-player-stage'),bar=e('div','ppt-player-bar'),prev=e('button','','上一页'),count=e('span','',''),next=e('button','','下一页'),fitb=e('button','','适合'),full=e('button','','全屏');root.classList.add('is-ui-hidden');bar.appendChild(prev);bar.appendChild(count);bar.appendChild(next);bar.appendChild(fitb);bar.appendChild(full);root.appendChild(stage);root.appendChild(bar);" +
      ...
      "function fit(){var v=vp(),vw=Math.max(1,v.w),vh=Math.max(1,v.h),por=vw<=720&&vh>vw,fill=mode==='fill'&&!por,sc=(fill?Math.max:Math.min)(vw/w,vh/h);stage.style.width=w+'px';stage.style.height=h+'px';stage.style.left=(v.x+vw/2)+'px';stage.style.top=(v.y+vh/2)+'px';stage.style.transform='translate(-50%,-50%) scale('+Math.max(.1,sc)+')';stage.setAttribute('data-scale-mode',fill?'fill':'fit')}" +
      "function fitMode(){mode=mode==='fill'?'fit':'fill';fitb.textContent=mode==='fill'?'填满':'适合';fit();chrome()}" +
