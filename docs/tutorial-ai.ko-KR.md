...
- CSS, 스크립트, 자유 HTML, PPTX XML을 생성하지 않습니다.
- 구조화된 레이아웃 필드를 우선 사용합니다. 주석, 보조 콜아웃, 사람이 편집한 결과 보존이 필요할 때만 `textBoxes` 를 소량 사용합니다.
...
- 캔버스에 삽입한 컴포넌트는 `slides[].objects[]` 에 저장됩니다.
- 기존 deck 을 수정할 때는 `id` 와 `type` 으로 대상 개체를 찾고 필요한 `data`, `x/y/w/h`, `style` 또는 요청된 필드만 수정합니다.
- 필요하지 않으면 `objects` 배열 전체를 다시 만들지 마세요. 사람이 조정한 위치, 크기, 레이어가 사라질 수 있습니다.
- 개체 단위 검사는 중복 id, 잘못된 위치/크기, 로컬 미디어 경로, 차트 값, 표 행/열 불일치를 잡아냅니다.
- 이미지, 비디오, 오디오, 차트, 표 개체는 Typed Inspector 로 사람이 계속 편집할 수 있으므로 데이터를 구조화되고 읽기 쉽게 유지하세요.
