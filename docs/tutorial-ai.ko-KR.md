...
6. 사용자가 `检查` 를 실행한 뒤 보고서를 보내면 `ERROR`, `WARNING`, `TIP` 순서로 수정합니다.
...
- `version` 은 `"0.1"` 로 유지합니다.
- 자유 형식 HTML로 바꾸지 않습니다.
- `slides[2].layout` 또는 `slides[4].metrics` 같은 경로를 따라 수정합니다.
- 수정된 전체 deck JSON을 반환합니다.
