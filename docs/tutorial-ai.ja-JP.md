...
- 人間が編集して作成した `canvas` の位置オフセットは、既に存在する場合は保持します。ユーザーが位置調整を明示的に求めない限り、大量の `canvas` 座標を生成しないでください。
...
`canvas` は、人間がキャンバス上で要素をドラッグしたときの軽い位置調整を保存します。キーは `title`、`subtitle`、`cards.0.title`、`table.rows.1.2` のような構造化フィールドパスです。AI が文言を修正する場合は既存の `canvas` をできるだけ保持してください。ユーザーが「レイアウトをリセット」「テンプレート位置に戻す」と求めた場合だけ削除します。
