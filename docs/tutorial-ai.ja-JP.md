...
- CSS、スクリプト、自由な HTML、PPTX XML は生成しない。
- 構造化されたレイアウトフィールドを優先します。注釈、補足コールアウト、人間の編集結果を保つ必要がある場合だけ、`textBoxes` を少量使用します。
...
- キャンバスに挿入されたコンポーネントは `slides[].objects[]` に保存されます。
- 既存 deck を編集する場合は、`id` と `type` で対象オブジェクトを見つけ、必要な `data`、`x/y/w/h`、`style` だけを変更します。
- 必要がない限り `objects` 配列全体を作り直さないでください。人間が調整した位置、サイズ、レイヤーが失われる可能性があります。
- オブジェクト単位の検証では、重複 id、不正な位置やサイズ、ローカルメディアパス、グラフ数値、表の行列不一致を検出します。
- 画像、動画、音声、グラフ、表のオブジェクトは型別 Inspector で人間も編集するため、データは構造化して読みやすく保ってください。
