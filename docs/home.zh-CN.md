...
v0.2.1 增加了结构化图表页：柱状图、折线图、环形图既可以在编辑器里可视化编辑，也可以由 AI 生成 `chart` JSON。v0.2 开始，PPT.html Studio 不只是一个格式原型，而是一条从模板、可视化编辑、AI JSON、质量检查到单文件分享的完整创作流程。
