...
v0.2.1 adds structured chart slides: bar, line, and donut charts can be edited visually or generated by AI as `chart` JSON. Since v0.2, PPT.html Studio is no longer just a format prototype; it has a complete creation workflow from template to validation to a shareable `.ppt.html` file.
