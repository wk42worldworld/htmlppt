...
- 人类在画布插入的组件会保存在 `slides[].objects[]`。
- 修改已有文稿时，优先按对象 `id` 和 `type` 找到目标对象，只改 `data`、`x/y/w/h`、`style` 或必要字段。
- 不要无故重建整个 `objects` 数组；否则会丢失人类调整过的位置、尺寸和层级。
- 对象会被质量检查校验：重复 id、错误几何、坏媒体路径、图表数值、表格行列不一致都会进入报告。
- 图片、视频、音频、图表和表格对象也可以由人类在类型化面板中继续编辑，所以字段应保持结构化和可读。
