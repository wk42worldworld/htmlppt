...
`objects` 是可选字段，用于保存当前页上的可视化对象。它适合由人类像 PPT/Keynote 一样插入、拖拽、缩放、复制、粘贴和调整层级，也适合 AI 按稳定对象 `id` 做局部编辑。
...
- `id`：对象稳定 ID。AI 修改对象时优先按 `id` 定位，不要依赖数组下标。
- `type`：对象类型，支持 `image`、`video`、`audio`、`chart`、`table`、`cards`、`metrics`、`timeline`、`quote`、`code`、`compare`、`shape`。
- `x`、`y`：对象左上角在 1280 × 720 幻灯片坐标中的位置。
- `w`、`h`：对象宽高。
- `rotation`：旋转角度，单位为度。
- `zIndex`：图层顺序，数值越大越靠前。
...
形状 `kind` 支持 `rectangle`、`roundedRectangle`、`ellipse`、`line`、`arrow`、`callout`。
...
- `fontFamily` 使用安全字体族 token：`system`、`display`、`arial`、`helvetica`、`aptos`、`calibri`、`verdana`、`tahoma`、`avenir`、`serif`、`georgia`、`times`、`cambria`、`cjk-sans`、`pingfang`、`yahei`、`dengxian`、`simhei`、`stheiti`、`hiragino-sans`、`noto-sans-cjk`、`cjk-serif`、`songti`、`simsun`、`fangsong`、`kaiti`、`yu-mincho`、`noto-serif-cjk`、`yu-gothic`、`meiryo`、`malgun`、`mono`、`menlo`、`consolas`、`courier`、`handwriting`。不要写任意 CSS 字体字符串。
...
- 画布：标题、卡片、指标、表格等可双击直接编辑；插入对象会保存到 `objects`，并可拖动、缩放、复制、粘贴、右键操作和调整层级。
